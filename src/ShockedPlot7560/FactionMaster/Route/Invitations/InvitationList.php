...
class InvitationList implements Route {
    ...
    public function __invoke(Player $player, UserEntity $User, array $UserPermissions, ?array $params = null)
    {
        ...
        $this->Collection = ButtonFactory::get(JoinInvitationListCollection::SLUG)->init($player, $User, $Invitations);
        $message = "";
        if (isset($params[0])) $message = $params[0];
        if (count($Invitations) == 0) $message .= Utils::getText($this->UserEntity->name, "NO_PENDING_INVITATION");
