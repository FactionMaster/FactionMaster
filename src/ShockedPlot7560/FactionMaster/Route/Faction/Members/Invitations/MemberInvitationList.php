...
class MemberInvitationList implements Route {
    ...
    public function __invoke(Player $player, UserEntity $User, array $UserPermissions, ?array $params = null)
    {
        ...
        $this->Collection = ButtonFactory::get(MemberInvitationListCollection::SLUG)->init($player, $User, $this->Invitations);
        if (isset($params[0])) $message = $params[0];
        if (count($this->Invitations) == 0) $message .= Utils::getText($User->name, "NO_PENDING_INVITATION");
