...
class PacketHooker implements Listener {
						...
						foreach($pk->commandData as $commandName => $commandData) {
							...
							if($cmd instanceof BaseCommand) {
								foreach($cmd->getConstraints() as $constraint){
									if(!$constraint->isVisibleTo($p->getPlayer())){
										continue 2;
									}
								}
								$pk->commandData[$commandName]->overloads = self::generateOverloads($p->getPlayer(), $cmd);
							}
						}
	 ...
	 */
	private static function generateOverloads(CommandSender|Player $cs, BaseCommand $command): array {
		$overloads = [];

		$scEnumName = $command->getName() . "SubCommands";
		$scEnumValue = [];
		...
		$scEnum = new CommandEnum($command->getName() . "SubCommands", $scEnumValue);
