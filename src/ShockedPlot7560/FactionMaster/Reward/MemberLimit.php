...
class MemberLimit extends Reward implements RewardInterface {
    ...
    public function executeGet(string $factionName, ?int $value = null) : bool {
        ...
        return MainAPI::updateFactionOption($factionName, 'max_player', $this->value);
    }

    public function executeCost(string $factionName, ?int $value = null) {
        if ($value !== null) $this->setValue($value);
        return ($result = MainAPI::updateFactionOption($factionName, 'max_player', $this->value * -1)) === false ? "ERROR" : $result;
    }
