...
class AcceptMemberToFac extends Button {
	public function __construct(InvitationEntity $request) {
		$this->setSlug(self::ACCEPT_MEMBER_TO_FAC_REQUEST)
								...
								Utils::newMenuSendTask(new MenuSendTask(
									...
									function () use ($request, $player, $faction, $message) {
										...
										Utils::newMenuSendTask(new MenuSendTask(
											...
											function () use ($player) {
												Utils::processMenu(RouterFactory::get(RouteSlug::JOIN_REQUEST_RECEIVE_ROUTE), $player, [Utils::getText($player->getName(), "ERROR")]);
											}
										));
									},
									function () use ($player) {
